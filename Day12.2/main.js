...
class Node {
  constructor(row, col) {
    ...
    if (this.elevation == -14) this.elevation = 0;
...
for (let row = 0; row < input.length; ++row) {
  ...
  for (let col = 0; col < input[row].length; ++col) {
    ...
    if (input[row][col] == 0) startingNodes.push(thisNode);
...
console.log(startingNodes.filter(node => node.distanceToEnd).map(node => node.distanceToEnd));
